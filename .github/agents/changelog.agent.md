---
description: "Use when generating changelogs from git log output. Produces publication-ready CHANGES file entries from raw git log --stat output following nginx changelog conventions."
...
### Step B: Ask the User to Select the Previous Release Tag

Use the ask-questions tool to present the discovered tags as clickable selectable options. Each tag should be an option the user can click to select. The most recent tag should be marked as the recommended default. Do not list tags as plain text — they must be interactive selectable options.

### Step C: Ask the User to Select the Current Release Tag

After the user selects the previous release tag, use the ask-questions tool again to present a filtered list of only the tags that come **after** the selected previous release tag in version order. Also include "HEAD (latest unreleased commits)" as an additional option marked as the recommended default. Each tag must be a clickable selectable option.

The changelog will cover all commits between the selected start and end points.

### Step D: Run the Git Log

Once the user selects both tags, run:
